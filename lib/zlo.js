...
Zlo.prototype._loadFromNet = function () {
    ...
    return new Promise(function(resolve, reject) {
        this._execLimited('npm install', function(err, stdout) {
            ...
            this._execLimited(bowerPath + ' install', function(err, stdout) {
                if (err) {
...
Zlo.prototype.killMD5 = function (options) {
    ...
    var killLocalPromise = !options.local ? Promise.resolve() : this._removeCacheFiles(this._cacheFileNames),
        killSvnPromise = !options.svn ?
            Promise.resolve() :
    ...
    Promise.all([killSvnPromise, killLocalPromise])
        .then(function() {
...
Zlo.prototype.killAll = function (options) {
    ...
    var killLocalPromise = !options.local ? Promise.resolve() : this._removeAllCacheFiles(),
        killSvnPromise = !options.svn ?
            Promise.resolve() :
    ...
    Promise.all([killLocalPromise, killSvnPromise])
        .then(function() {
    ...
    if (this._postinstall && this._postinstall.length > 0) {
        ...
        return Promise.all(this._postinstall.map(function(postinstall) {
            return new Promise(function(resolve, reject) {
                ...
                }.bind(this)).stdout.on('data', function(data) {
                    verboseLog(data);
                });
            }.bind(this));
        }, this))
