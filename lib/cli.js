...
var config = fs.readJsonSync('zlo-config.json'),
    target = {},
    packageData = fs.existsSync('npm-shrinkwrap.json') ? fs.readJsonSync('npm-shrinkwrap.json') : fs.readJsonSync('package.json'),
    zlo = new Zlo(config, packageData, { verbose: program.verbose, dev: program.dev, loadTimeout: config.loadTimeout });
